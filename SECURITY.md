...
1. A capability's Plan carries **quantified expectations** (`expects`): which assets may leave (with maximums), which must arrive (with minimums), which approvals may be granted (token, spender, cap), which NFT ids may leave or must arrive, and any exact canonical ERC-721/1155 receipts the write must emit. Risk labels classify the danger; expects make it mechanically auditable ([ADR 0004](./docs/adr/0004-quantified-expects-in-plans.md)).
2. **Simulation** replays the unsigned transactions against live chain state via `debug_traceCall` (dual tracers, state chained across transactions and across plans — [ADR 0002](./docs/adr/0002-simulation-via-debug-tracecall.md)) and extracts actual effects: ERC-20/721/1155 asset movement, exact canonical NFT transfer receipts (including zero values and self-transfers), approvals, operator grants, **native MON flows from call frames** (they emit no events), wrapped-native mints/burns.
3. **Effects reconciliation** warns on any *undeclared* difference or missing exact receipt: undeclared outflow, outflow above the declared max, undeclared or over-cap approval, unmet minimum inflow, undeclared NFT id or per-id ERC-1155 amount, missing NFT inflow, or an absent/mismatched required ERC-721/1155 receipt; it also warns on any NFT operator grant, plan tampering (`planHash` mismatch), or revert.
