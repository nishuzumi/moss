...
Alpha。Monad 主网（chain id 143）。已支持协议：WMON（wrap/unwrap/balanceOf）、erc20 通用协议（任意代币转账/余额/授权查询，含原生 MON）、erc721 通用协议（任意 NFT 转账/归属查询）、erc1155 通用协议（任意多代币 collection 的转账/余额查询）、Kuru（市价单 swap、报价、市场列表）。
