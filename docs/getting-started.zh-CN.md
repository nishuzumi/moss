...
在把未签名交易交给钱包之前，应将每一个结构化 Outcome 与调用工具前 记录的用户意图逐项比较： - 确认模拟没有中止，并且所有结果都不包含 Warning； - 确认 `operation` 和 `protocol` 与用户请求一致； - 确认 sender 和 receiver 地址正确； - 使用规范的资产身份核查输入和输出资产，而不是仅依赖用户提供的 Token 符号； - 确认金额、精度单位和资产流动方向； - 确认滑点、最小输出、截止时间和授权额度等限制； - 确认嵌套 Capability 只引入了预期的前置交易； - 确认结构化 Outcome 和有序 Receipt 证据描述了预期的状态变化。 如果任何字段无法与记录的用户意图相互验证，应在签名前停止流程， 并请用户检查不一致之处。 模拟成功只能说明在模拟状态下发生了什么，以及观察到的 Change 是否 得到解析。它不能证明用户已经同意，也不能自动证明结果符合用户意图。 在请求签名前，应同时展示未签名交易和经过核查的 Outcome 摘要。
