...
One quote request is bounded in the work it may ask of the chain, not only in the routes it may consider. The allowance covers the whole request — it is opened before the first read and verification is charged to it, so the bound is not merely on the quoting stage that follows. Routes are evaluated by a fixed number of workers rather than all at once, and calls are counted against that allowance and a per-route cap, charged at the leg so a route that priced two legs before a third refused to encode has still spent two. Exhausting either is reported as an unmeasured route under its own reason, never as an answer, so a write refuses it by default rather than trading an unbounded call bill for a comparison. Without this one advisory quote over the 256-route maximum came to roughly ten thousand calls issued in a single 256-wide burst, against whatever endpoint the operator configured.
