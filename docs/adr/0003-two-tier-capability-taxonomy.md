...
- **Verbs `open` and `close`; direction is a parameter.** A position's lifecycle is two writes, not four — closing a long and closing a short are one user-perspective semantic. Direction rides a closed `side` parameter, matching the precedent that a swap's fixed side is a parameter rather than two verbs. `long`/`short` as verbs would force `closeLong`/`closeShort` and reintroduce exactly the synonym decay the closed set exists to prevent.
...
- `open` and `close` are position-lifecycle verbs, not perps-only ones: any Protocol whose write opens or closes a persistent position carrying its own liquidation surface may use them, while the collateral that backs the position stays on `supply`/`withdraw`.
- A word enters a closed set with a stated reason and the docs that enumerate it — this ADR, the `CONTEXT.md` glossary, and the `discover` schema derived from `VERBS`/`CATEGORIES`/`RISK_LABELS` — updated in the same change.
