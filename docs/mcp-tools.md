...
- `warnings` — effects reconciliation output. Codes: `REVERTED`, `PLAN_TAMPERED`, `UNDECLARED_OUTFLOW`, `OUTFLOW_EXCEEDS_MAX`, `UNDECLARED_APPROVAL`, `APPROVAL_EXCEEDS_MAX`, `MIN_INFLOW_NOT_MET`, `UNDECLARED_NFT_OUT`, `NFT_OUT_EXCEEDS_MAX`, `REQUIRED_NFT_OUT_MISSING` (a declared ERC-721/1155 outflow emitted no matching canonical transfer receipt), `NFT_OPERATOR_GRANTED`, `CONFIRMATION_MISSING` (a receipt the plan's `confirms` declared did not appear). Warnings fire only on **undeclared differences**, except a declared NFT outflow also requires its canonical receipt; a declared fungible outflow with nothing back (an unstake request, margin posting) is legitimate.
