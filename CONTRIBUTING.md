...
- [ ] Protocol-specific write receipts are declared with `@Event` and gated via `confirms`. ERC-721/1155 capabilities may instead declare an exact canonical audit-plane receipt in `expects.nftTransfers` (kind, collection, from/to, id, exact amount/operator where applicable); other standard events still need an authored receipt when required ([ADR 0008](./docs/adr/0008-observation-plane.md)).
