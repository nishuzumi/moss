import { createHandle } from "@themoss/core";
...
describe("Pendle swap builder", () => {
  it("builds a direct buy-PT Router transaction with an empty simple route", () => {
    const plan = buildPendleSwapPlan(BUY_QUOTE, RECEIVER, router);
  ...
  it("builds a direct sell-PT Router transaction with an empty simple route", () => {
    const plan = buildPendleSwapPlan(SELL_QUOTE, RECEIVER, router);
  ...
  it("requires approving the Router to spend the input token", () => {
    expect(buildPendleSwapPlan(BUY_QUOTE, RECEIVER, router).approval).toEqual({
      token: UNDERLYING,
      spender: ROUTER,
      amount: 1_000_000n,
    });
    expect(buildPendleSwapPlan(SELL_QUOTE, RECEIVER, router).approval).toEqual({
      token: PT,
      spender: ROUTER,
      amount: 1_000_000n,
    });
