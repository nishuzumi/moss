import type { AddressValue, Handle, MossRuntime } from "@themoss/core";
...
export async function verifyPendleMarket(
  ...
  factory?: Handle<typeof PendleMarketFactoryAbi>,
): Promise<VerifiedMarket> {
  return verifyMarketCandidate(
    runtimeReader(runtime, factory),
    candidate,
    candidate.expectedUnderlying,
  );
}
...
function runtimeReader(
  ...
  return {
    ...
    isValidMarket: (market) =>
      factory
        ? factory.read.isValidMarket([market])
        : client.readContract({
