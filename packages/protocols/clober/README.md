...
The current Moss Receipt contract does not pass Capability parameters or the transaction sender into a parser, so the user is inferred from the required input debit. The parser still relies on the Simulator to supply the complete, correctly attributed Change set; it performs no external reads or trace reconstruction.
